...
Popup = {
    ...
    _getOffset: function(element) {
        ...
        // show delete without spilling off screen
        // added 090316
        offset.top = offset.top - 50;
        var popupWidth = 300 + 15;
        var topVal;
        if ($(element).attr('id') === 'cal') {
            return function() {
                // inject css to lessenpopup for w and h
                setTimeout(function() { 
                    $('.lessenpopup').css('width', '98%');
                    $('.lessenpopup').css('height', '80%');
                }, 10);
                return {
                    left: 10,
                    top: 10,
            ...
            $('.lessenpopup').css('width', '');
            $('.lessenpopup').css('height', '');
        };

        if ($element.hasClass('js-due-date')) {
            return {
                left: $(window).width() - (popupWidth * 2)
            };
        };

        topVal = $element.hasClass('header-member') ? 10 : offset.top + $element.outerHeight() - 50;
